...
export const REGION_NAME_TO_CODE: Record<string, string> = Object.entries(
    REGION_CODE_TO_NAME,
).reduce(
    (acc, [code, name]) => {
        acc[name] = code
 ...
 * 通过地区代码获取对应的地区名称
 * @param code 地区代码
 * @returns 地区名称，如果不存在则返回 undefined
 */
...
export function getRegionsByLevel(level: number): Region[] {
    ...
    for (const [code, name] of Object.entries(REGION_CODE_TO_NAME)) {
        if (getRegionLevel(code) === level) {
...
/**
 * 根据地区代码构建完整的地区树
 * 从给定的地区代码一直获取上级地区，直到最顶级（一级行政区）
 * @param code 地区代码
 * @returns 地区树数组，从一级行政区到当前级别，如果代码不存在则返回空数组
 */
 ...
 * 构建地区代码到父代码的映射缓存
 */
let regionParentCodeCache: Record<string, string | undefined> | null = null

/**
 * 构建完整的地区代码父子关系映射
 */
function buildRegionParentCodeMap(): Record<string, string | undefined> {
    if (regionParentCodeCache) {
        return regionParentCodeCache
    }

    const map: Record<string, string | undefined> = {}

    for (const code of Object.keys(REGION_CODE_TO_NAME)) {
        let parentCode: string | undefined = undefined
        const level = getRegionLevel(code)

        if (level === RegionLevel.THIRD_LEVEL) {
            // 三级行政区：先尝试二级代码，再尝试一级代码
            const secondLevelCode = code.substring(0, 4) + '00'
            if (REGION_CODE_TO_NAME[secondLevelCode]) {
                parentCode = secondLevelCode
            } else {
                const firstLevelCode = code.substring(0, 2) + '0000'
                if (REGION_CODE_TO_NAME[firstLevelCode]) {
                    parentCode = firstLevelCode
                }
            }
        } else if (level === RegionLevel.SECOND_LEVEL) {
            // 二级行政区：获取一级代码
            const firstLevelCode = code.substring(0, 2) + '0000'
            if (REGION_CODE_TO_NAME[firstLevelCode]) {
                parentCode = firstLevelCode
    ...
    regionParentCodeCache = map
    return map
}

/**
 * 规范化地区代码 - 将各种格式的地区标识符转换为标准的数字代码
 * @param input 地区标识符（可以是代码、名称或路径）
 * @returns 标准化的地区代码，如果无法识别则返回原始输入
 */
export function normalizeRegionCode(input: string): string {
    ...
    // 1. 如果已经是有效的地区代码，直接返回
    if (REGION_CODE_TO_NAME[cleanInput]) {
        return cleanInput
    }

    // 2. 如果是地区名称，转换为代码
    if (REGION_NAME_TO_CODE[cleanInput]) {
        return REGION_NAME_TO_CODE[cleanInput]
    }

    // 3. 尝试处理路径格式（如 "浙江省 > 杭州市 > 西湖区"）
    const separators = [' > ', '/', '，', ',', '-', '|']
    for (const sep of separators) {
        if (cleanInput.includes(sep)) {
            const parts = cleanInput.split(sep).map(p => p.trim()).filter(p => p)
            // 取最后一个部分（最详细的地区），尝试转换
            for (let i = parts.length - 1; i >= 0; i--) {
                const part = parts[i]
                if (part) {
                    const code = REGION_NAME_TO_CODE[part]
                    if (code) {
                        return code
                    }
                }
    ...
    if (digitMatch && digitMatch[0]) {
        const digitStr = digitMatch[0]
        if (REGION_CODE_TO_NAME[digitStr]) {
            return digitStr
        }
    }
...
/**
 * 根据地区代码获取完整的地区路径字符串
 * @param code 地区代码、名称或路径
 * @param separator 分隔符，默认为 ' > '
 * @returns 地区路径字符串，例如："浙江省 > 杭州市 > 西湖区"
 */
export function getRegionPath(input: string, separator: string = ' > '): string {
    if (!input || typeof input !== 'string') {
        return ''
    }

    // 规范化输入以获取标准的地区代码
    const regionCode = normalizeRegionCode(input)
    
    if (!regionCode || !REGION_CODE_TO_NAME[regionCode]) {
        return input
    }

    const parentMap = buildRegionParentCodeMap()
    const path: string[] = []
    let currentCode: string | undefined = regionCode
    ...
    while (currentCode) {
        ...
        const regionName = REGION_CODE_TO_NAME[currentCode]
        if (regionName) {
            path.unshift(regionName)
        }
        
        currentCode = parentMap[currentCode]
    }

    return path.length > 0 ? path.join(separator) : input
}
