...
<script setup lang="ts">
    ...
    import { fetchOrderDetail, getOrderStatusInfo, OrderType } from '@/api/order'
    ...
    const getStatusInfo = computed(() => {
        return () => {
            ...
            const orderType = determinOrderType()
    ...
    const determinOrderType = () => {
        if (orderDetail.value == undefined) {
            throw Error('orderDetail is undefined')
        }
        return orderDetail.value.storeOrders && orderDetail.value.storeOrders.length > 1
            ? OrderType.PARENT
