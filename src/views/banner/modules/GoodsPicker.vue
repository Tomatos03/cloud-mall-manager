<template>
    ...
    >
        <!-- 搜索区域 -->
        <div
            class="flex items-center gap-3 mb-6 p-4 bg-[#f8fafc] rounded-xl border border-gray-100"
        >
            <el-input
                v-model="searchText"
        ...
        <!-- 表格区域 -->
        <div
            class="border border-gray-100 rounded-xl overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.02)]"
        >
            <el-table
                :data="list"
...
<script setup lang="ts">
    ...
    import type { GoodsListItem } from '@/api/goods'
    ...
    const selectedRow = ref<GoodsListItem | null>(null)
    ...
    const loadData = async () => {
        try {
            ...
            // 过滤出上架商品
            list.value = (res.data.records || []).filter((r: GoodsListItem) => r.status === true)
            total.value = res.data.total

            if (selectedId.value) {
                const found = list.value.find((r) => r.goodsId === selectedId.value)
    ...
    const onRowClick = (row: GoodsListItem) => {
        selectRow(row)
    }

    const selectRow = (row: GoodsListItem) => {
        selectedRow.value = row
        selectedId.value = row.goodsId
    }
    ...
    const handleConfirm = () => {
        ...
        emit('confirm', {
            goodsId: String(selectedRow.value.goodsId),
