<template>
  <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-6">
    ...
    <el-table :data="topProducts" style="width: 100%" :header-cell-style="{ background: '#f8faff' }">
      <el-table-column label="排名" width="80" align="center">
        <template #default="{ row }">
          <div :class="['w-8 h-8 rounded-full flex items-center justify-center mx-auto text-sm font-bold',
            row.rank <= 3 ? 'bg-orange-100 text-orange-600' : 'bg-gray-100 text-gray-500']">
            {{ row.rank }}
          </div>
        </template>
      </el-table-column>
      <el-table-column label="商品信息" min-width="300">
        <template #default="{ row }">
          <div class="flex items-center gap-3">
            <el-image 
              :src="getImageURL(row.goodsCover)" 
      ...
      <el-table-column prop="saleAmount" label="销售额" width="150" align="right" sortable>
        <template #default="{ row }">
          <span class="font-bold text-orange-600">¥{{ formatPrice(row.saleAmount) }}</span>
        </template>
      </el-table-column>
...
import type { TopProduct } from '@/api/common/statistics'
...
// 格式化价格：将分转换为元，并使用本地化格式
const formatPrice = (fen: string | number): string => {
