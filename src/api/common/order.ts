...
export interface OrderGoodsItem {
  ...
  goodsPrice: number // 商品价格（单位：分）
  goodsPriceText: string // 商品价格文本（单位：元）
  quantity: number // 数量
  totalPrice: number // 小计（单位：分）
  totalPriceText: string // 小计文本（单位：元）
...
export interface OrderStoreItem {
  ...
  totalPrice: number // 该店铺订单总价（单位：分）
  totalPriceText: string // 该店铺订单总价文本（单位：元）
  count: number // 商品总数
}
...
export interface OrderDetail {
  ...
  totalPrice: number // 订单总价（单位：分）
  totalPriceText: string // 订单总价文本（单位：元）
}
 ...
 * 子订单/普通订单状态映射 - 商家视角
 */
export const MerchantOrderStatusMap: Record<
  string,
  { label: string; type: 'warning' | 'primary' | 'success' | 'info' | 'danger' }
> = {
 ...
 * 子订单/普通订单状态映射 - 管理员视角
 */
export const AdminOrderStatusMap: Record<
  string,
  { label: string; type: 'warning' | 'primary' | 'success' | 'info' | 'danger' }
> = {
  CREATED: { label: '待支付', type: 'warning' },
 ...
 * @param status 订单状态码
 * @param roleType 角色类型: 'admin' 管理员, 'merchant' 商家，默认为管理员视角
 */
export function getOrderStatusInfo(
  ...
  // 子订单和普通订单根据角色选择状态映射
  const statusMap = role === 'admin' ? AdminOrderStatusMap : MerchantOrderStatusMap
  return statusMap[status] || { label: '-', type: 'info' as const }
}
