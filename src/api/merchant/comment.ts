...
import type { OrderItem } from '../common/order'
...
export function fetchCommentPage(params: CommentPageParams) {
    return http.get<CommentPageResult>(`${PREFIX}/comments`, params)
}
...
export function replyComment(commentId: string, replyContent: string) {
    return http.post(`${PREFIX}/comments/reply`, { commentId, replyContent })
}
...
export function getCommentDetail(orderNo: string) {
    return http.get<CommentItem>(`${PREFIX}/comments/${orderNo}`)
}
